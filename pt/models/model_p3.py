...
class Model_P3(BaseModel):
	...
	def generate_variables(self):
		...
		# self.vars.register('czxf', Helper.g('zxf_{},{},{}', 'C', 0, r_max, n_candidate_stops+1, n_candidate_stops+1))

		# self.vars.register('cxx', Helper.g('cxx_{},{},{}', 'B', 0, r_max, n_candidate_stops, n_candidate_stops))
		...
		# self.vars.register('ts', Helper.g('ts_{}', 'C', -10, 1))
		
		self.vars.register('w', Helper.g('w_{},{}', 'C', reduce(lambda a,b: a+b, demands), len(demands), len(demands)))
		# self.vars.register('w', Helper.g('w_{},{}', 'C', 0, len(demands), len(demands)))
		...
		self.vars.register('xadm', Helper.g('xadm_{},{},{},{}', 'B', 0, r_max, n_candidate_stops+1, n_candidate_stops+1, n_candidate_stops+1))
		self.vars.register('xadm2', Helper.g('xadm2_{},{},{}', 'B', 0, r_max, n_candidate_stops+1, n_candidate_stops+1))
		...
		# Shortest path between OD pair
		self.vars.register('st', Helper.g('st_{},{},{},{}', 'C', reduce(lambda a,b: a+b, demands) * (n_candidate_stops * n_candidate_stops), len(demands), len(demands), n_candidate_stops, n_candidate_stops))
	...
	def generate_constraints(self):
		...
		# 					   (vars.x[r][i+1][j+1], -ridetime(i, j))] + 
		# 					  [(vars.x[r][i+1][x+1], -ridetime(i, x))
		...
		# print(list(map(lambda x: ridetime(x[0], x[1]), combinations(range(len(candidate_stops)), 2))))

		# max_T = sum(list(map(lambda x: ridetime(x[0], x[1]), combinations(range(len(candidate_stops)), 2))))
		...
		# 	for i in range(n_candidate_stops + 1):
		# 		for j in range(n_candidate_stops + 1):
		# 			cns.append('C7-4(%d)' % idx)
		# 			cs.append((
		# 				linex([(vars.czxf[r][i][j],  1),
		# 					   (vars.zxf[r][i][j],  -1)]),
		# 				'L', 0, 0))

		# 			cns.append('C7-5(%d)' % idx)
		# 			cs.append((
		# 				linex([(vars.czxf[r][i][j],  1),
		# 					   (vars.c[r]       ,   60)]),
		# 				'L', 60, 0))

		# 			cns.append('C7-6(%d)' % idx)
		# 			cs.append((
		# 				linex([(vars.czxf[r][i][j],  1),
		# 					   (vars.zxf[r][i][j],  -1),
		# 					   (vars.c[r],   60)]),
		...
		for r in range(r_max):
			for i in range(n_candidate_stops+1):
				for j in range(n_candidate_stops+1):
					cns.append('2XADM1(%d)' % idx)
					cs.append((
						linex([(vars.xadm2[r][i][j],  1),
							   (vars.x[r][i][0],       -1)]),
						'L', 0, 0))

					cns.append('2XADM2(%d)' % idx)
					cs.append((
						linex([(vars.xadm2[r][i][j],  1),
							   (vars.adm[0][j],        -1)]),
						'L', 0, 0))

					cns.append('2XADM4(%d)' % idx)
					cs.append((
						linex([(vars.xadm2[r][i][j],  1)] + 
							  [(vars.x[r][k+1][n_candidate_stops + 1], 1)
							   for k in range(n_candidate_stops)]),
							   # (vars.c[r],           -1)]),
						'L', 1, 0))

					cns.append('2XADM3(%d)' % idx)
					cs.append((
						linex([(vars.xadm2[r][i][j],  1),
							   (vars.adm[0][j],      -1)] + 
							  [(vars.x[r][k+1][n_candidate_stops + 1], 1)
		...
		# cns.append('test')
		# cs.append((linex([(vars.x[0][11][12], 1)]), 'E', 1, 0))
